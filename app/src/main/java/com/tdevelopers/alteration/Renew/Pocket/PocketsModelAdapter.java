package com.tdevelopers.alteration.Renew.Pocket;
...
import com.tdevelopers.alteration.Renew.Pocket.PocketResponses.PocketItem;
...
public class PocketsModelAdapter extends RecyclerView.Adapter<PocketsModelAdapter.PocketsModelViewHolder> {

    private List<PocketItem> data;

    private ImageView itemImage;
    private ImageView itemImagegarments;

    public PocketsModelAdapter(List<PocketItem> data, ImageView itemImage,ImageView itemImagegarments) {
        this.data = data;
    ...
    @Override
    public PocketsModelViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new PocketsModelViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.model_item, parent, false));
    }

    @Override
    public void onBindViewHolder(final PocketsModelViewHolder holder, int position) {

        try {
            final PocketItem current = data.get(position);
            if (current != null) {
                ...
                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        if (current.itemFlag) {
                            ...
                            NewRenewActivity.style.remove(current.pocket_style);
                            NewRenewActivity.id.remove(current.id);
                            NewRenewActivity.price.remove(current.price);
                            NewRenewActivity.Cat.remove("Pocket");
                            ...
                            if(NewRenewActivity.Cat.contains("Pocket"))
                            {
                                int a= NewRenewActivity.Cat.indexOf("Pocket");
                                ...
                                NewRenewActivity.style.set(a,current.pocket_style);
                                NewRenewActivity.id.set(a,current.id);
                                NewRenewActivity.price.set(a,current.price);
                                NewRenewActivity.Cat.set(a,"Pocket");
                            }
                            else
                            {
                                ...
                                NewRenewActivity.style.add(current.pocket_style);
                                NewRenewActivity.id.add(current.id);
                                NewRenewActivity.price.add(current.price);
                                NewRenewActivity.Cat.add("Pocket");
                            }
                            NewRenewActivity.total_price=0.0;
                            for (int i = 0; i < NewRenewActivity.price.size(); i++) {
                                NewRenewActivity.total_price = NewRenewActivity.total_price + Double.parseDouble(NewRenewActivity.price.get(i));
    ...
    class PocketsModelViewHolder extends RecyclerView.ViewHolder {

        TextView name;
        ImageView pic;

        public PocketsModelViewHolder(View itemView) {
            super(itemView);
