package com.tdevelopers.alteration.Renew.Placket;
...
import com.tdevelopers.alteration.Renew.Placket.PlacketResponses.PlacketItem;
...
public class PlacketsModelAdapter extends RecyclerView.Adapter<PlacketsModelAdapter.PlacketsModelViewHolder> {

    private List<PlacketItem> data;

    private ImageView itemImage;
    private ImageView itemImagegarments;

    public PlacketsModelAdapter(List<PlacketItem> data, ImageView itemImage,ImageView itemImagegarments) {
        this.data = data;
    ...
    @Override
    public PlacketsModelViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new PlacketsModelViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.model_item, parent, false));
    }

    @Override
    public void onBindViewHolder(final PlacketsModelViewHolder holder, int position) {

        try {
            final PlacketItem current = data.get(position);
            if (current != null) {
                ...
                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        if (current.itemFlag) {
                            ...
                            NewRenewActivity.style.remove(current.placket_style);
                            NewRenewActivity.id.remove(current.id);
                            NewRenewActivity.price.remove(current.price);
                            NewRenewActivity.Cat.remove("Placket");
                            ...
                            if(NewRenewActivity.Cat.contains("Placket"))
                            {
                                int a= NewRenewActivity.Cat.indexOf("Placket");
                                ...
                                NewRenewActivity.style.set(a,current.placket_style);
                                NewRenewActivity.id.set(a,current.id);
                                NewRenewActivity.price.set(a,current.price);
                                NewRenewActivity.Cat.set(a,"Placket");
                            }
                            else
                            {
                                ...
                                NewRenewActivity.style.add(current.placket_style);
                                NewRenewActivity.id.add(current.id);
                                NewRenewActivity.price.add(current.price);
                                NewRenewActivity.Cat.add("Placket");
                            }
                            NewRenewActivity.total_price=0.0;
                            for (int i = 0; i < NewRenewActivity.price.size(); i++) {
                                NewRenewActivity.total_price = NewRenewActivity.total_price + Double.parseDouble(NewRenewActivity.price.get(i));
    ...
    class PlacketsModelViewHolder extends RecyclerView.ViewHolder {

        TextView name;
        ImageView pic;

        public PlacketsModelViewHolder(View itemView) {
            super(itemView);
